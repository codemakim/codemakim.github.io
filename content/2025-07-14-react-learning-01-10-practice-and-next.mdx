...
목표: Props와 리스트 렌더링을 활용하여 재사용 가능한 컴포넌트를 만든다.
...
목표: 조건부 렌더링을 사용하여 완료/미완료 상태를 표시한다.
...
- `completed`가 `true`면 체크마크 표시하고 텍스트에 취소선
- `completed`가 `false`면 일반 텍스트
- 삼항 연산자 또는 `&&` 연산자 사용

### 3. 상품 필터링

목표: `filter()`와 `map()`을 연결하여 데이터를 필터링하고 표시한다.
...
이제 UI를 표현하는 방법을 배웠으니, 다음 챕터에서는 인터랙티비티 추가하기 (Adding Interactivity)를 학습한다.
