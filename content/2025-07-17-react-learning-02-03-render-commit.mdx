...
date: "2025-07-17"
description: "React의 UI 업데이트 과정인 렌더링과 커밋을 이해한다"
...
React가 컴포넌트를 렌더링하기 시작하는 이유는 두 가지이다.
...
React가 컴포넌트 함수를 호출해서 무엇을 화면에 그려야 할지 계산한다.
...
React는 실제 DOM을 직접 수정하지 않고, 먼저 가상 DOM을 만듭니다.
...
React가 변경사항을 찾아내는 과정을 Reconciliation이라고 한다.
동작 방식:
...
function Image() {
  return <img src="https://i.imgur.com/ZF6s192.jpg" alt="Photo" />;
}
```
...
Reconciliation에서 계산한 변경사항을 실제 DOM에 적용하는 단계이다.
커밋 단계의 작업:
...
중요: React는 렌더링 간에 차이가 있을 때만 DOM 노드를 변경한다.
변경 없으면 DOM 건드리지 않음:
...
- 입력한 텍스트 사라짐
- 포커스 잃음
  React는 필요한 부분만 업데이트하여 이런 문제를 방지한다!
...
Vue도 유사한 가상 DOM (Virtual DOM) 방식을 사용하지만, 몇 가지 차이가 있다:
공통점:
...
- Vue: 반응성 시스템이 자동으로 변경 감지 (Proxy 기반)
  하지만 결과는 동일:
  둘 다 가상 DOM을 사용하여 변경된 부분만 효율적으로 업데이트한다!
