...
date: "2025-07-01"
description: "Vue 개발자가 Next.js를 빠르게 이해하도록 프로젝트 구조와 설정 파일을 상세히 설명한다"
tags: ["Next.js", "Vue", "초급", "프론트엔드"]
draft: true
---

# Next.js 15 프로젝트 가이드 (Vue 개발자를 위한)
...
Markdown + React 컴포넌트를 사용할 수 있는 블로그 포스트이다.
...
export const metadata: Metadata = {
  title: "개발 블로그",
  description: "Next.js와 웹 개발에 관한 이야기들을 공유한다",
};
// 루트 레이아웃 (모든 페이지에 적용)
...
Next.js에서 `useSearchParams`를 사용할 때 정적 빌드(`output: 'export'`)를 하면 다음과 같은 에러가 발생한다:
...
export default function HomeContent() {
  ...
  return (
    <>
      {/* 태그 필터 */}
      <div>
        <button onClick={() => handleTagSelect("React")}>React</button>
        <button onClick={() => handleTagSelect(null)}>전체</button>
      </div>

      {/* 필터링된 컨텐츠 */}
      <div>현재 선택된 태그: {selectedTag || "전체"}</div>
    </>
  );
}
```
