...
Ref의 실용적인 사용 예제를 살펴본다. 스톱워치를 만들 때 interval ID를 저장하는 데 ref를 사용한다.
...
**왜 `intervalRef`를 사용하나?**

- `setInterval`이 반환하는 ID를 저장해야 나중에 `clearInterval`로 중지할 수 있다
- 이 ID는 렌더링에 사용되지 않으므로 ref에 저장한다
- State로 저장하면 ID가 변경될 때마다 불필요한 리렌더링이 발생한다
