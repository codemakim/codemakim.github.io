...
1~9편은 컴포넌트 라우터 기반으로 “SPA 라우팅 실무 코어”를 다룬다. React Router v6.4+에서는 라우터가 데이터와 폼 제출까지 함께 다루는 Data Router 축이 실무 비중이 크다.

- 10편: Data Router 기초(`createBrowserRouter`, `loader`, `redirect`, `errorElement`)
- 11편: Form/Mutation(`action`, `<Form>`, `useNavigation`, `useFetcher`)
- 12편: 운영/배포/테스트(정적 호스팅 새로고침 404, `basename`, 테스트 패턴)
