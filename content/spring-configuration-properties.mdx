...
description: "YAML 설정을 타입 안전하게 바인딩하는 방법과 @Value와의 차이, 실무에서 DB/Redis/AWS 설정을 관리하는 패턴을 정리했습니다."
date: "2025-09-28"
...
Spring Boot 애플리케이션의 설정을 관리하는 방법은 `@Value`와 `@ConfigurationProperties` 두 가지가 있습니다. 실무에서 DB, Redis, AWS 같은 외부 시스템 설정을 다룰 때는 `@ConfigurationProperties`를 주로 사용합니다. 타입 안전성과 검증 기능을 제공하기 때문입니다.

## Value의 한계

간단한 설정은 `@Value`로 충분합니다.
...
Lombok을 사용하면 getter/setter 보일러플레이트를 제거할 수 있습니다.
...
생성자 바인딩을 사용하면 불변 객체로 만들 수 있습니다.
...
Spring Boot 3.0부터는 `@ConstructorBinding`이 자동으로 적용되므로 생략 가능합니다.
...
관련된 설정이 여러 개면 `@ConfigurationProperties`가 낫습니다.
...
중첩 구조는 `@ConfigurationProperties`가 훨씬 낫습니다.
...
JSR-303 Bean Validation을 사용해서 설정값을 검증할 수 있습니다.
...
복잡한 검증 로직은 커스텀 Validator를 만들 수 있습니다.
...
Spring Boot Configuration Processor를 추가하면 IDE에서 자동완성을 지원합니다.
...
주석을 작성하면 IDE에서 설명을 볼 수 있습니다.

## 정리

`@ConfigurationProperties`는 관련된 설정을 그룹화하고 타입 안전하게 관리할 수 있는 방법입니다. 단순한 값 하나는 `@Value`가 낫지만, DB나 Redis 같은 외부 시스템 설정처럼 여러 값이 모여있고 검증이 필요한 경우에는 `@ConfigurationProperties`를 사용하는 것이 좋습니다.

Validation을 함께 사용하면 잘못된 설정으로 애플리케이션이 시작되는 것을 방지할 수 있고, IDE 자동완성 지원으로 개발 경험도 좋아집니다. 실무에서는 설정 클래스를 따로 패키지로 분리해서 관리하고, 환경별로 다른 YAML 파일을 사용하는 것이 일반적입니다.
