...
description: "필드 주입, 세터 주입, 생성자 주입의 차이점과 생성자 주입을 권장하는 이유, Lombok과의 관계를 정리했습니다."
date: "2025-09-21"
...
Spring에서 의존성을 주입하는 방법은 필드 주입, 세터 주입, 생성자 주입 세 가지가 있습니다. 실무에서는 생성자 주입을 권장하는데 그 이유와 Lombok을 활용한 방법을 정리했습니다.

## 의존성 주입의 세 가지 방식

### 필드 주입

가장 간단해 보이지만 권장하지 않는 방식입니다.
...
의존성을 나중에 변경할 수 있어서 불변성이 보장되지 않습니다.

### 생성자 주입

Spring 4.3부터 권장하는 방식입니다.
...
생성자가 하나만 있으면 `@Autowired`도 생략할 수 있습니다.
...
필드 주입은 코드가 짧아서 편해 보이지만 여러 문제를 가지고 있습니다.

### 불변성을 보장하지 못함

`final` 키워드를 사용할 수 없어서 의존성이 중간에 바뀔 수 있습니다.
...
생성자 주입은 `final`을 사용해서 불변성을 보장합니다.
...
순수 자바 코드로 테스트할 때 의존성을 주입할 방법이 없습니다.
...
필드 주입은 리플렉션을 사용하거나 Spring 컨테이너를 띄워야 테스트가 가능합니다.

### 순환 참조 감지가 늦음

필드 주입은 런타임에 순환 참조를 발견하지만, 생성자 주입은 애플리케이션 시작 시점에 발견합니다.
...
필드 주입은 이 코드가 실행되기 전까지 문제를 모릅니다. 생성자 주입이었다면 애플리케이션 시작이 실패해서 즉시 알 수 있습니다.
...
필드 주입은 의존성을 계속 추가해도 코드가 짧아서 문제를 못 느낍니다.
...
생성자 주입은 생성자가 길어져서 클래스가 너무 많은 책임을 가지고 있다는 신호를 줍니다.
...
이 경우 클래스를 분리하거나 책임을 나누는 리팩토링이 필요합니다.

## Autowired vs Inject

`@Autowired`는 Spring 전용이고 `@Inject`는 Java 표준입니다.

### Autowired

Spring Framework에서 제공하는 애노테이션입니다.
...
`required` 속성으로 필수 여부를 지정할 수 있습니다.
...
실무에서는 Spring을 사용하고 있으니 `@Autowired`를 쓰거나 생성자 주입에서는 아예 생략하는 것을 권장합니다.

## Lombok과 생성자 주입

생성자 주입은 코드가 길어지는데 Lombok을 사용하면 간결하게 작성할 수 있습니다.

### RequiredArgsConstructor

`final` 필드만 포함하는 생성자를 자동으로 생성해줍니다.
...
이렇게 하면 필드 주입처럼 간결하면서도 생성자 주입의 장점을 모두 가질 수 있습니다.
...
의존성 주입에는 `@RequiredArgsConstructor`를 사용합니다.

### NonNull과 함께 사용

`@NonNull`이 붙은 필드도 생성자에 포함됩니다.
...
하지만 의존성 주입에서는 대부분 `final`만 사용합니다.
...
Spring의 Bean은 생명주기를 가지고 있고 스코프로 관리됩니다.

### Singleton 스코프

기본 스코프입니다. 애플리케이션 전체에서 하나의 인스턴스만 생성됩니다.
...
같은 Bean을 여러 곳에서 주입받아도 모두 같은 인스턴스입니다.
...
생성자 주입으로는 Prototype Bean을 제대로 사용할 수 없습니다. 생성자는 한 번만 호출되기 때문입니다.
...
Prototype Bean이 필요하면 `Provider`나 `ObjectFactory`를 사용합니다.
...
HTTP 요청마다 새로운 인스턴스를 생성합니다. 웹 애플리케이션에서만 사용할 수 있습니다.
...
`proxyMode`를 설정해야 Singleton Bean에 주입할 수 있습니다.
...
요청마다 다른 `RequestContext` 인스턴스가 사용됩니다.
...
같은 세션의 요청은 같은 인스턴스를 공유합니다.

### Application 스코프

ServletContext 생명주기와 같습니다. Singleton과 비슷하지만 웹 애플리케이션 전체에서 하나만 생성됩니다.
...
상태를 가지는 Bean을 Singleton으로 만들면 동시성 문제가 발생합니다.
...
이런 경우 상태를 제거하거나 다른 스코프를 사용해야 합니다.
...
`@Qualifier`로 특정 Bean을 선택할 수 있습니다.
...
Map으로 받으면 Bean 이름을 키로 사용할 수 있습니다.
...
Bean을 실제로 사용할 때까지 생성을 미루고 싶을 때 사용합니다.
...
Spring의 의존성 주입은 생성자 주입을 기본으로 사용하는 것이 좋습니다. Lombok의 `@RequiredArgsConstructor`를 함께 사용하면 코드도 간결해집니다. 필드 주입은 테스트가 어렵고 불변성을 보장하지 못하며 순환 참조를 늦게 발견하는 등 여러 문제가 있어서 피하는 것이 좋습니다.

Bean 스코프는 대부분 Singleton을 사용하고, 특별한 경우에만 Prototype, Request, Session 같은 다른 스코프를 고려합니다. 상태를 가지는 Bean은 동시성 문제를 일으킬 수 있으므로 주의해야 합니다.
