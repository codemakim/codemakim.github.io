...
React는 선언적으로 UI를 관리하지만, 때로는 DOM에 직접 접근해야 할 때가 있습니다.
...
동적으로 생성되는 여러 요소에 ref를 연결하려면 어떻게 할까요? Ref 콜백을 사용합니다.
...
기본적으로 컴포넌트는 자신의 DOM을 노출하지 않습니다.
...
DOM 전체를 노출하는 대신, 필요한 메서드만 노출할 수 있습니다.
...
Ref는 언제 DOM 노드를 참조하게 될까요? React의 렌더링 주기를 이해해야 합니다.
...
State 업데이트가 비동기적으로 처리되어 DOM이 즉시 업데이트되지 않을 때가 있습니다. `flushSync`로 동기적 업데이트를 강제할 수 있습니다.
