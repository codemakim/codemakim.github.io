...
description: "자바스크립트 배열에서 자주 사용되는 메서드들의 특징과 사용법을 정리했다."
date: "2021-03-17"
...
자바스크립트에서 배열을 다룰 때 자주 사용되는 메서드들을 정리했다. 각 메서드의 특징과 주의사항, 실제 사용 예제를 포함하고 있다.
...
배열의 모든 요소가 조건을 만족하는지 테스트한다.

### 원본 배열을 변경하지 않다

### 특징

- 빈 배열에서 호출하면 항상 true를 반환
- 조건을 만족하지 않는 요소를 찾으면 즉시 false 반환하고 순회 중단
...
배열의 요소 중 하나라도 조건을 만족하는지 테스트한다.

### 원본 배열을 변경하지 않다

### 특징

- 빈 배열에서 호출하면 항상 false를 반환
- 조건을 만족하는 요소를 찾으면 즉시 true 반환하고 순회 중단
...
조건을 만족하는 모든 요소를 모아 새로운 배열을 반환한다.
...
### 원본 배열을 변경하지 않다 (단, callback이 변경할 수는 있음)
...
배열의 모든 요소에 함수를 적용한 결과로 새로운 배열을 생성한다.
...
배열의 각 요소에 대해 reducer 함수를 실행하여 하나의 값으로 축약한다.
...
배열의 기존 요소를 삭제하거나 교체하고 새 요소를 추가한다.
...
이제 훨씬 읽기 쉽고 실용적인 자바스크립트 배열 메서드 가이드가 되었다!
