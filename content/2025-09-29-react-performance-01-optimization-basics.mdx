...
date: "2025-09-29"
description: "React 렌더링 3단계 프로세스와 React.memo, useMemo, useCallback의 동작 원리를 이해한다"
...
"React에서 리렌더링은 3가지 경우에 발생한다: State 변경, Props 변경, 부모 리렌더링.

핵심 문제:
부모가 리렌더링되면 기본적으로 모든 자식 컴포넌트가 리렌더링된다. Props가 변경되지 않았어도요.

왜 이렇게 설계됐나?
안전성 우선이다. React는 모든 컴포넌트를 리렌더링하여 UI가 항상 최신 state를 반영하도록 보장한다.
...
주의:
과도한 최적화는 오히려 성능 저하. Profiler로 측정 후 병목 지점만 최적화해야 한다."
