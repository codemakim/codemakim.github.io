...
description: "Spring Framework에서 컨트롤러 메서드 파라미터를 활용하는 다양한 방법과 애노테이션들을 정리했다."
date: "2025-09-07"
...
Spring MVC에서 컨트롤러 메서드의 파라미터를 다루는 다양한 방법과 애노테이션들을 정리했다. 각 애노테이션의 특징과 올바른 사용법, 주의사항을 포함하고 있다.

## @PathVariable

URL 경로의 변수를 메서드 파라미터로 받는 애노테이션이다.
...
URL에 일치하지 않는 타입의 값이 들어오면 HTTP 400 Bad Request가 발생한다.
...
HTTP 요청 파라미터(쿼리 스트링, 폼 데이터)를 메서드 파라미터로 받는 애노테이션이다.
...
HTTP 요청과 함께 전달된 쿠키 값을 받는 애노테이션이다.
...
HTTP 요청 헤더 정보를 메서드 파라미터로 받는 애노테이션이다.
...
@ModelAttribute와 함께 사용하여 바인딩 오류를 처리한다.
...
HTTP 요청 본문을 객체로 변환하여 받는 애노테이션이다.
...
JSR-303 Bean Validation을 사용하여 객체를 검증한다.

### 기본 검증 애노테이션

```java
public class UserCreateRequest {
    @NotBlank(message = "이름은 필수이다")
    @Size(min = 2, max = 50, message = "이름은 2-50자 사이여야 한다")
    private String name;
    ...
    @NotNull(message = "나이는 필수이다")
    @Min(value = 1, message = "나이는 1 이상이어야 한다")
    @Max(value = 150, message = "나이는 150 이하여야 한다")
    private Integer age;
