...
Effect가 너무 자주 재실행된다면 의존성을 줄일 수 있는지 검토하세요.

### 불필요한 의존성 제거

가장 흔한 3가지 문제와 해결법입니다.
...
Effect 내부에서 객체를 생성하면 불필요한 의존성을 피할 수 있습니다.
...
이전 state를 기반으로 새 state를 계산할 때는 업데이터 함수를 사용하세요.
...
최신 값은 읽고 싶지만 Effect를 재실행하고 싶지 않다면 Effect Event를 사용하세요.
