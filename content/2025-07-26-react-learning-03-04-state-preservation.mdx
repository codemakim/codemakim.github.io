...
React는 **컴포넌트의 위치**를 기준으로 state를 보존하거나 재설정합니다. 위치는 다음 두 가지로 결정됩니다:
...
따라서 React는 **다른 위치**로 인식하여 각각 독립적인 state를 가집니다.

### 위치 판단의 핵심 규칙

React는 렌더링할 때 다음과 같이 위치를 판단합니다:

1. **JSX 표현식의 위치가 중요합니다**
   - 같은 부모 안에서도 **다른 줄/다른 표현식**이면 다른 위치
   - 삼항연산자(`? :`)는 **하나의 표현식**이므로 같은 위치
...
- **다른 순서**이므로 React는 **다른 위치**로 인식
- 각각 독립적인 state를 가짐

**동작:**
- 두 Counter는 **별도로 카운트**됨 (독립적인 state)
...
삼항연산자는 **하나의 표현식**이므로, 조건에 따라 다른 값을 반환하더라도 **같은 위치**로 인식됩니다.
...
- `isFancy`가 변경되어도 **같은 위치**에 **같은 컴포넌트 타입** (`Counter`)이 렌더링됨
- React는 이를 **동일한 인스턴스**로 인식하여 state를 보존
...
같은 위치라도 **컴포넌트 타입이 다르면** React는 다른 컴포넌트로 인식하여 state를 재설정합니다.
...
- `isPaused`가 변경되면 이전 컴포넌트를 **완전히 제거**하고 새로운 컴포넌트를 **새로 생성**
...
같은 위치의 같은 컴포넌트인데 state를 초기화하고 싶다면 어떻게 할까요? 두 가지 방법이 있습니다.

#### 방법 1: 다른 위치에 렌더링

**원리:** JSX에서 **독립적인 표현식**으로 작성하면 React는 **다른 위치**로 인식합니다.
...
- **다른 위치**이므로 React는 다른 컴포넌트로 인식
- 플레이어 전환 시 state가 재설정됨

**주의:** 삼항연산자를 사용하면 같은 위치로 인식되어 state가 보존됩니다:
```jsx
{isPlayerA ? <Counter person="Taylor" /> : <Counter person="Sarah" />}
```
이 경우는 **하나의 표현식**이므로 같은 위치(0번째 자식)로 인식됩니다.

#### 방법 2: key로 state 재설정 (권장)

**원리:** `key` prop은 React가 컴포넌트를 식별하는 데 사용됩니다. 같은 위치라도 **`key`가 다르면** 다른 컴포넌트로 인식합니다.
