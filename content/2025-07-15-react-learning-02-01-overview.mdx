...
date: "2025-07-15"
description: "React에서 사용자 상호작용을 처리하는 기본 개념과 이벤트 핸들러를 학습한다"
...
화면의 일부 요소는 사용자 입력에 응답하여 업데이트된다.
...
React에서는 시간에 따라 변하는 데이터를 `state`라고 부른다.
...
React에서는 JSX에 이벤트 핸들러를 추가하여 사용자 상호작용에 응답할 수 있다.

이벤트 핸들러는 다음과 같은 사용자 동작에 응답하는 함수다:
...
3. `onClick={handleClick()}` - 함수를 호출하지 말아야 한다

### 이벤트 핸들러 작성 패턴

이벤트 핸들러를 작성하는 여러 방법이 있다.
...
export default function Toolbar() {
  return (
    <div>
      <AlertButton message="영상을 재생합니다!">영상 재생</AlertButton>
      <AlertButton message="이미지를 업로드합니다!">이미지 업로드</AlertButton>
    </div>
  );
}
```
...
부모 컴포넌트가 자식의 이벤트 핸들러를 지정하고 싶을 때 유용하다.
...
function PlayButton({ movieName }) {
  ...
  return <Button onClick={handlePlayClick}>"{movieName}" 재생</Button>;
}
...
export default function App() {
  return (
    <div>
      <Button onSmash={() => alert("영상 재생!")}>영상 재생</Button>
      <Button onSmash={() => alert("이미지 업로드!")}>이미지 업로드</Button>
    </div>
  );
}
```
...
이벤트 핸들러는 자식 컴포넌트의 이벤트도 포착한다. 이를 "버블링" 또는 "전파"라고 한다.
...
export default function Toolbar() {
  return (
    ...
    >
      <button onClick={() => alert("영상 재생!")}>영상 재생</button>
      <button onClick={() => alert("이미지 업로드!")}>이미지 업로드</button>
    </div>
  );
}
```
...
이제 버튼 클릭 시 부모의 `onClick`은 실행되지 않는다.

### 기본 동작 방지하기

일부 브라우저 이벤트는 기본 동작이 있다. `e.preventDefault()`로 방지할 수 있다.
...
이벤트 핸들러는 순수할 필요가 없다. 부작용을 일으키기에 가장 좋은 곳이다.
