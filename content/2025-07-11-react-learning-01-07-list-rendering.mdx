...
#### 3. **렌더링 중에 key를 생성하지 마세요**

```jsx
// ❌ 렌더링할 때마다 새로운 key 생성 (최악!)
// 컴포넌트가 렌더링될 때마다 다른 key가 생성되어 
// React가 항목을 추적할 수 없음
{items.map(item =>
...
**핵심**: key는 **렌더링 중에 생성하면 안 되고**, **데이터를 생성할 때 한 번만 할당**해야 합니다.
...
const [items, setItems] = useState([
  { id: uuidv4(), text: 'Item 1' },  // 초기화 시 한 번만 생성
  { id: uuidv4(), text: 'Item 2' },  // 초기화 시 한 번만 생성
]);
...
const addItem = (text) => {
  setItems([
    ...items,
    { id: uuidv4(), text }  // 아이템 추가 시점에 UUID 생성
  ]);
};
...
return (
  <ul>
    {items.map(item =>
      <li key={item.id}>{item.text}</li>  // 이미 할당된 id 사용
    )}
  </ul>
);
```
...
**왜 이 방법이 올바른가?**
- UUID는 **데이터를 생성하는 시점**에 한 번만 생성
- 생성된 ID는 **데이터 객체에 저장**되어 렌더링 시마다 동일한 값 사용
...
return (
  <ul>
    {items.map(item =>
      <li key={crypto.randomUUID()}>{item.text}</li>  // 매번 다른 key!
    )}
  </ul>
);
```
