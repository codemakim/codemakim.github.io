...
firstName과 lastName을 합쳐서 fullName을 만들고, 이를 별도의 state로 관리하려는 코드다.
...
수천 개의 할 일 목록을 필터링하고 정렬하는 코드로, 컴포넌트가 리렌더링될 때마다 이 무거운 계산이 반복된다.
...
사용자가 다른 프로필로 이동할 때, 이전 프로필에서 작성하던 댓글을 초기화하려는 코드다.
...
목록(items)이 변경될 때, 현재 선택된 항목(selection)을 초기화하려는 코드다.
...
상품이 장바구니에 담길 때마다 알림을 표시하려는 코드다.
...
폼 방문 시점과 입력 필드 수정 시점을 분석 서버로 전송하려는 코드다.
...
카드 게임에서 골드 카드를 3개 모으면 라운드가 올라가고, 5라운드가 되면 게임이 종료되는 로직을 구현한 코드다.
...
앱이 시작될 때 로컬 스토리지에서 데이터를 불러오고 인증 토큰을 확인하려는 코드다.
...
토글 버튼의 상태(On/Off)가 변경될 때마다 부모 컴포넌트에 알리려는 코드다.
...
자식 컴포넌트에서 API로 데이터를 가져와서 부모 컴포넌트로 전달하려는 코드다.
