...
- 월/수/금에만 수행하는 습관 → 화요일에는 목록에 표시되지 않음
- 시작일이 내일인 습관 → 오늘은 목록에 표시되지 않음
- 종료일이 어제인 습관 → 오늘은 목록에 표시되지 않음

**필터/정렬 기능 (선택사항, 향후 구현):**

습관이 많아질 때(10개 이상) 유용한 기능:

- **필터 옵션**:
  - 전체 습관 보기 (기간과 무관하게 모든 습관)
  - 활성 습관만 (현재 기간 내 습관)
  - 완료된 습관만 (오늘 완료한 습관)
  - 미완료 습관만 (오늘 완료하지 않은 습관)
  - 색상별 필터 (특정 색상 습관만)
...
- **정렬 옵션**:
  ...
  - 오래된순 (생성일 기준)
  - 완료율순 (완료율 높은 순)
  - 연속 달성 일수순 (연속 달성 일수 높은 순)
  - 제목순 (알파벳/한글 순)

**현재 상태**: 구현되지 않음 (습관이 적으면 불필요)
...
- **날짜 수정 시 문제**: 시작일/종료일 변경 시 기존 기록이 새로운 기간 밖에 있으면 논리적으로 맞지 않음
- **요일 수정 시 문제**: 요일 변경 시 기존 기록의 날짜가 새로운 요일과 맞지 않으면 논리적 불일치 발생
- **안전한 수정**: 제목, 설명, 색상은 기록과 무관하므로 안전하게 수정 가능

**향후 확장 가능성:**

사용자 요구가 있을 경우, 날짜/요일 수정 기능을 추가할 수 있으나 다음 검증이 필요:

- 시작일 수정: 기존 기록 중 가장 이른 날짜보다 늦추지 않기
- 종료일 수정: 기존 기록 중 가장 늦은 날짜보다 앞당기지 않기
- 요일 수정: 기존 기록 삭제 후 재시작 옵션 제공
...
- React Hooks (로컬 상태)
- React Context API (HabitsProvider - 습관 데이터 전역 캐시)
  - 메모리 기반 캐시 (React state)
  - **모든 습관을 캐싱**: `fetchAllHabits()`는 날짜 필터링 없이 사용자의 모든 습관을 가져옴 (과거 습관 포함)
  - 중복 패칭 방지
  - 생성/수정/삭제 시 캐시 자동 업데이트
...
4. ✅ 습관 상세 화면
   ...
   - ✅ 삭제 기능 (HabitsProvider 캐시 업데이트 포함, 확인 모달 포함)
   - ✅ 수정 기능 (기본 수정만: 제목, 설명, 색상)
