...
4. **상태 라벨**:
   - 종료일이 지난 습관(`end_date < 오늘`)에는 상단에 **"완료"** 라벨을 표시
...
2. **습관 데이터 패칭**:
   ...
   - **날짜 필터링 없이 전체 습관을 사용**하되, 종료일 기준 내림차순 정렬 (최신순)
   - 과거 습관(`end_date < 오늘`) 여부는 카드 내부에서 상태 라벨("완료")로만 구분
...
1. **아카이브 카드 컴포넌트** (`PastHabitCard.tsx`):
   - 현재 `HabitCard`와 유사하지만 완료 체크 버튼 없음
   - 통계 정보 표시
   - 종료일이 지난 습관에는 상단에 **"완료"** 라벨 표시
   - 클릭 시 상세 페이지로 이동
2. **페이지 레이아웃**:
   - 헤더 (제목, 뒤로가기 링크)
   - 습관 카드 그리드
...
- [x] 전체 습관 사용 + 종료일 기준 내림차순 정렬
- [x] 종료일이 지난 습관에 대해 카드 상단 "완료" 라벨 표시
- [x] 각 습관별 `habit_records` 패칭 로직 구현
- [x] 통계 계산 함수 구현 (HabitStats 로직 재사용)
- [x] `PastHabitCard` 컴포넌트 생성
- [x] 반응형 그리드 레이아웃 적용 (모바일 1열, 데스크톱 2열)
- [x] 빈 상태 처리 ("등록된 습관이 없습니다")
