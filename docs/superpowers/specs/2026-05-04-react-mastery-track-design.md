...
- 매핑 방식 (택1, 1차 결정):
  - **(a)** 토픽 정의(`topics.ts`)에 토픽별로 `legacyPostSlugs: string[]`를 손으로 적어둔다. **1차 추천**: 단순하고 콘텐츠 작성과 동시에 매핑이 자연스럽다.
  - **(b)** 기존 mdx frontmatter에 `mastery: { topic: "...", role: "intro" }` 같은 메타를 추가하여 자동 매핑한다. v1.5 이후로 미룬다.
